...
export class TradeExecutionEngine {
  ...
  private performRiskChecks(order: TradeOrder): { passed: boolean; reason?: string } {
    ...
    // Check order value
    const orderValue = order.price ? order.amount * order.price : order.amount * 50000 // Estimate for market orders
