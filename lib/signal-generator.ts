...
export class SignalGenerator {
  ...
  private async generateSignals(configs: SignalConfig[]): Promise<GeneratedSignal[]> {
      ...
      for (const symbol of config.symbols) {
        for (const timeframe of config.timeframes) {
          try {
            ...
            const signal = await strategy.generateSignal(symbol, timeframe, marketData, indicators, config)
  ...
  private async calculateIndicators(marketData: MarketData): Promise<TechnicalIndicators> {
  ...
  async generateSignal(
    ...
    if (macd.macd > macd.signal && macd.histogram > 0 && macd.macd > 0) {
      return {
        ...
        timeframe,
        indicators,
        reasoning: "MACD bullish crossover confirmed with positive histogram and signal above zero line.",
      }
    }
    ...
    if (macd.macd < macd.signal && macd.histogram < 0 && macd.macd < 0) {
      return {
        ...
        timeframe,
        indicators,
        reasoning: "MACD bearish crossover confirmed with negative histogram and signal below zero line.",
      }
    }
  ...
  async generateSignal(
    ...
    if (bandWidth < 0.1 && volume.ratio > 1.5) {
      ...
      return {
        ...
        confidence: 78,
        entryPrice: currentPrice,
        targetPrice: type === "BUY" ? bollinger.upper * 1.02 : bollinger.lower * 0.98,
...
class SupportResistanceStrategy extends SignalStrategy {
  async generateSignal(
    ...
    // Bounce from support
    if (currentPrice > sma.sma50 * 0.995 && currentPrice < sma.sma50 * 1.005 && rsi < 45) {
      return {
        ...
        timeframe,
        indicators,
        reasoning: "Price bouncing from 50-day SMA support with RSI showing oversold conditions.",
      }
    }

    // Rejection from resistance
    if (currentPrice > sma.sma20 * 0.995 && currentPrice < sma.sma20 * 1.005 && rsi > 55) {
      return {
        strategy: "Support/Resistance",
        symbol,
        type: "SELL" as const,
        ...
        timeframe,
        indicators,
        reasoning: "Price rejected at 20-day SMA resistance with RSI showing overbought conditions.",
      }
    }
