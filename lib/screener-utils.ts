...
export const defaultColumns: ScreenerColumn[] = [
  ...
  { key: "volatility", label: "Volatility", type: "percentage", sortable: true },
  { key: "beta", label: "Beta", type: "number", sortable: true },
]

export const formatValue = (value: any, type: ScreenerColumn["type"]): string => {
...
export const exportToCSV = (data: any[], columns: string[], filename = "screener-export.csv") => {
...
export const saveScreenerConfig = (config: ScreenerFilter, name: string) => {
  const savedConfigs = JSON.parse(localStorage.getItem("screenerConfigs") || "{}")
