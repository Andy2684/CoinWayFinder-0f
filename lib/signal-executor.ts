...
export class SignalExecutor {
  ...
  private async performPreExecutionChecks(signal: any): Promise<{ passed: boolean; reason?: string }> {
    ...
    const existingPosition = Array.from(this.activePositions.values()).find((pos) => pos.symbol === signal.symbol)
  ...
  private selectBestExchange(symbol: string): string {
    ...
    const availableExchanges = this.config.exchangePreferences.filter((exchange) => {
  ...
  private estimateFees(quantity: number, price: number, exchange: string): number {
  ...
  private async setStopLoss(signal: any, quantity: number, exchange: string): Promise<void> {
    try {
      const adapter = ExchangeAdapterFactory.getAdapter(exchange)
      if (!adapter) return
  ...
  private async setTakeProfit(signal: any, quantity: number, exchange: string): Promise<void> {
    try {
      const adapter = ExchangeAdapterFactory.getAdapter(exchange)
      if (!adapter) return
