...
export function filterSignals(signals: Signal[], filters: any): Signal[] {
  return signals.filter((signal) => {
    ...
    // Strategy filter
    if (filters.strategies?.length > 0 && !filters.strategies.includes(signal.strategy)) {
      return false
    }

    // Exchange filter
    if (filters.exchanges?.length > 0 && !filters.exchanges.includes(signal.exchange)) {
      return false
    }
    ...
    // Risk level filter
    if (filters.riskLevels?.length > 0 && !filters.riskLevels.includes(signal.riskLevel)) {
      return false
    }
