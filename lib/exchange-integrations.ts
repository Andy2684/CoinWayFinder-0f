...
export class ExchangeAPI {
  ...
  constructor(exchangeId: string, credentials: ExchangeCredentials) {
    ...
    this.baseUrl = credentials.testnet && config.testnetUrl ? config.testnetUrl : config.baseUrl
  }

  private createSignature(method: string, path: string, body = "", timestamp: string): string {
    ...
    switch (this.config.id) {
      ...
      case "bybit":
        const bybitMessage = timestamp + this.credentials.apiKey + "5000" + body
        return crypto.createHmac("sha256", secretKey).update(bybitMessage).digest("hex")

      case "kucoin":
        const kucoinMessage = timestamp + method + path + body
        return crypto.createHmac("sha256", secretKey).update(kucoinMessage).digest("base64")

      case "okx":
        const okxMessage = timestamp + method + path + body
        return crypto.createHmac("sha256", secretKey).update(okxMessage).digest("base64")

      case "coinbase":
        const cbMessage = timestamp + method + path + body
        return crypto.createHmac("sha256", Buffer.from(secretKey, "base64")).update(cbMessage).digest("base64")

      default:
        throw new Error(`Signature method not implemented for ${this.config.id}`)
    }
  }
  ...
  async makeRequest(method: string, endpoint: string, params: any = {}): Promise<any> {
    const path = endpoint
  ...
  createWebSocket(channels: string[]): WebSocket {
    const wsUrl = this.credentials.testnet && this.config.testnetWsUrl ? this.config.testnetWsUrl : this.config.wsUrl
...
export class ExchangeFactory {
  static create(exchangeId: string, credentials: ExchangeCredentials): ExchangeAPI {
...
export class PortfolioAggregator {
  ...
  private normalizeBalance(exchangeId: string, balance: any): Record<string, number> {
