...
export class PaperTradingEngine {
  ...
  private updateAccountEquity(): void {
    const totalUnrealizedPnL = Array.from(this.positions.values()).reduce(
    ...
    this.account.marginLevel =
      this.account.margin > 0
        ? (this.account.equity / this.account.margin) * 100
        : 0;
  }
  ...
  async placeOrder(
    orderRequest: Omit<
      PaperOrder,
      "id" | "status" | "filledQuantity" | "timestamp"
    >,
  ): Promise<PaperOrder> {
  ...
  private validateOrder(order: PaperOrder): {
    valid: boolean;
    reason?: string;
  } {
  ...
  private async executeOrder(order: PaperOrder): Promise<void> {
    ...
    if (order.type === "MARKET") {
      ...
      const slippageAmount =
        basePrice * this.config.slippage * (Math.random() - 0.5) * 2;
        ...
        if (order.quantity >= existingPosition.quantity) {
          ...
          if (order.quantity > existingPosition.quantity) {
            const remainingQuantity =
              order.quantity - existingPosition.quantity;
            const newPosition: PaperPosition = {
              symbol: order.symbol,
              side: order.side === "BUY" ? "LONG" : "SHORT",
    ...
    this.orders.forEach(async (order) => {
      ...
      switch (order.type) {
        ...
        case "STOP":
          if (
            order.side === "BUY" &&
            marketData.price >= (order.stopPrice || 0)
          ) {
            shouldExecute = true;
          } else if (
            order.side === "SELL" &&
  ...
  getPerformanceMetrics(): any {
    ...
    const totalReturn =
      ((this.account.equity - this.config.initialBalance) /
