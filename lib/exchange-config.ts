...
export const exchangeConfigs: Record<string, ExchangeConfig> = {
  binance: {
    ...
    orderTypes: ["MARKET", "LIMIT", "STOP_LOSS", "STOP_LOSS_LIMIT", "TAKE_PROFIT", "TAKE_PROFIT_LIMIT", "LIMIT_MAKER"],
...
export function buildAuthHeaders(
  ...
  switch (exchangeId) {
    case "binance":
      const binanceSignature = createHMACSignature(credentials.secretKey, `timestamp=${timestamp}${body || ""}`)
...
function createHMACSignature(secret: string, message: string, encoding: "hex" | "base64" = "hex"): string {
