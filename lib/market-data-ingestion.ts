...
export class MarketDataIngestion {
  private subscribers: Map<string, Set<(data: MarketDataPoint) => void>> =
    new Map();
  private orderBookSubscribers: Map<
    string,
    Set<(data: OrderBookData) => void>
  > = new Map();
  private tradeSubscribers: Map<string, Set<(data: TradeData) => void>> =
    new Map();
  ...
  private handleReconnection(
    ...
    if (attempts < this.maxReconnectAttempts) {
      ...
      setTimeout(
        () => {
          console.log(
            `Attempting to reconnect to ${exchangeId} (attempt ${attempts + 1})`,
          );
