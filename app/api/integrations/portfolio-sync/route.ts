...
import { PortfolioAggregator, ExchangeFactory } from "@/lib/exchange-integrations"
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!exchanges || !Array.isArray(exchanges)) {
      return NextResponse.json({ success: false, error: "Invalid exchanges data" }, { status: 400 })
    }
