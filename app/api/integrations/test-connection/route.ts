  ...
  try {
    ...
    if (!exchangeId || !credentials) {
      return NextResponse.json(
        { success: false, error: "Missing exchange ID or credentials" },
        { status: 400 },
      );
    }
    ...
    const baseUrl =
      testnet && config.testnetUrl ? config.testnetUrl : config.baseUrl;
    ...
    if (response.ok) {
      ...
      return NextResponse.json({
        ...
        data: {
          ...
          accountInfo: {
            hasBalance: Array.isArray(data.balances)
