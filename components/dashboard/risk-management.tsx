...
import { Shield, AlertTriangle, TrendingDown, Target, Settings, Activity, Zap } from "lucide-react"
...
export function RiskManagement() {
  ...
  const riskMetrics = [
    { label: "Portfolio Risk Score", value: 6.8, max: 10, color: "text-yellow-400" },
    { label: "Current Drawdown", value: 7.2, max: 10, color: "text-orange-400" },
    { label: "Position Concentration", value: 35, max: 50, color: "text-blue-400" },
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-gray-300 text-sm">Max Drawdown (%)</label>
                    <span className="text-white font-medium">{riskSettings.maxDrawdown}%</span>
                  </div>
                  <Slider
                    value={[riskSettings.maxDrawdown]}
                    onValueChange={(value) => setRiskSettings({ ...riskSettings, maxDrawdown: value[0] })}
                    max={20}
                    min={5}
                    step={1}
                    className="w-full"
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-gray-300 text-sm">Position Size Limit (%)</label>
                    <span className="text-white font-medium">{riskSettings.positionSize}%</span>
                  </div>
                  <Slider
                    value={[riskSettings.positionSize]}
                    onValueChange={(value) => setRiskSettings({ ...riskSettings, positionSize: value[0] })}
                    max={20}
                    min={1}
                    step={1}
                    className="w-full"
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-white font-medium">{riskSettings.riskPerTrade}%</span>
                  </div>
                  <Slider
                    value={[riskSettings.riskPerTrade]}
                    onValueChange={(value) => setRiskSettings({ ...riskSettings, riskPerTrade: value[0] })}
                    max={10}
                    min={0.5}
                    step={0.5}
                    className="w-full"
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-3 bg-gray-800/30 rounded-lg">
                  <div>
                    ...
                    <p className="text-gray-400 text-sm">Automatic stop loss orders</p>
                  </div>
                  <Switch
                    checked={riskSettings.stopLoss}
                    onCheckedChange={(checked) => setRiskSettings({ ...riskSettings, stopLoss: checked })}
                  />
                </div>
                ...
                <div className="flex items-center justify-between p-3 bg-gray-800/30 rounded-lg">
                  <div>
                    ...
                    <p className="text-gray-400 text-sm">Automatic profit taking</p>
                  </div>
                  <Switch
                    checked={riskSettings.takeProfit}
                    onCheckedChange={(checked) => setRiskSettings({ ...riskSettings, takeProfit: checked })}
                  />
                </div>

                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-gray-300 text-sm">Max Open Positions</label>
                    <span className="text-white font-medium">{riskSettings.maxOpenPositions}</span>
                  </div>
                  <Slider
                    value={[riskSettings.maxOpenPositions]}
                    onValueChange={(value) => setRiskSettings({ ...riskSettings, maxOpenPositions: value[0] })}
                    max={20}
                    min={1}
                    step={1}
                    className="w-full"
        ...
        <CardHeader className="pb-4">
          <div className="flex items-center justify-between">
            ...
            <Badge className="bg-red-500/10 text-red-400">
              {riskAlerts.filter((alert) => alert.severity === "high").length} Critical
            </Badge>
          </div>
        </CardHeader>
        <CardContent>
          ...
          <div className="mt-6 flex items-center justify-between">
            ...
            <Button className="bg-[#30D5C8] hover:bg-[#30D5C8]/90">Configure Alerts</Button>
          </div>
        </CardContent>
      </Card>
