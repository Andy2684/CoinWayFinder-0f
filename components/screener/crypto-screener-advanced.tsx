...
const presetFilters = {
  ...
  highVolume: {
    name: "High Volume",
    volume24h: [1000000000, Number.POSITIVE_INFINITY],
  },
  oversold: { name: "Oversold (RSI < 30)", rsi: [0, 30] },
  overbought: { name: "Overbought (RSI > 70)", rsi: [70, 100] },
  largeCap: {
    name: "Large Cap (>$10B)",
    marketCap: [10000000000, Number.POSITIVE_INFINITY],
  },
};
...
export function CryptoScreenerAdvanced() {
    ...
    return cryptoData
      .filter((asset) => {
        ...
        const matchesExchange =
          selectedExchange === "All" || asset.exchange === selectedExchange;
        const matchesPrice =
          asset.price >= priceRange[0] && asset.price <= priceRange[1];
        const matchesMarketCap =
          asset.marketCap >= marketCapRange[0] &&
          asset.marketCap <= marketCapRange[1];
        const matchesChange24h =
          asset.change24h >= change24hRange[0] &&
          asset.change24h <= change24hRange[1];
        const matchesVolume =
          asset.volume24h >= volumeRange[0] &&
          asset.volume24h <= volumeRange[1];
        const matchesRsi = asset.rsi >= rsiRange[0] && asset.rsi <= rsiRange[1];
        const matchesVolatility =
          asset.volatility >= volatilityRange[0] &&
          asset.volatility <= volatilityRange[1];
  ...
  const applyPresetFilter = (preset: keyof typeof presetFilters) => {
    ...
    if (filter.volume24h)
      setVolumeRange([
        filter.volume24h[0],
        filter.volume24h[1] === Number.POSITIVE_INFINITY
          ? 20000000000
          : filter.volume24h[1],
      ]);
    ...
    if (filter.marketCap)
      setMarketCapRange([
        filter.marketCap[0],
        filter.marketCap[1] === Number.POSITIVE_INFINITY
          ? 1000000000000
          : filter.marketCap[1],
      ]);
  };
  ...
  const exportToCSV = () => {
    ...
    const rows = filteredData
      .map((asset) =>
        selectedColumns.map((col) => asset[col as keyof CryptoAsset]).join(","),
      )
      .join("\n");
          ...
          <div className="flex flex-wrap gap-2">
            {Object.entries(presetFilters).map(([key, filter]) => (
              <Button
                ...
                size="sm"
                onClick={() =>
                  applyPresetFilter(key as keyof typeof presetFilters)
                }
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Showing {filteredData.length} assets matching your criteria
              </CardDescription>
            </div>
            <div className="flex items-center gap-2">
              ...
              <TableHeader>
                <TableRow className="border-gray-800">
                  ...
                  {selectedColumns.includes("change7d") && (
                    <TableHead className="text-gray-300">7d Change</TableHead>
                  )}
                  ...
                  {selectedColumns.includes("volume24h") && (
                    <TableHead className="text-gray-300">Volume 24h</TableHead>
                  )}
                  {selectedColumns.includes("rank") && (
                  ...
                  >
                    ...
                    {selectedColumns.includes("price") && (
                      <TableCell className="text-white">
                        {formatNumber(asset.price, "currency")}
                      </TableCell>
                    )}
                    {selectedColumns.includes("change24h") && (
                      <TableCell>
                    ...
                    {selectedColumns.includes("change7d") && (
                      <TableCell>
                        <div
                          className={`flex items-center ${asset.change7d >= 0 ? "text-green-400" : "text-red-400"}`}
                        >
                          {asset.change7d >= 0 ? (
                            <TrendingUp className="h-4 w-4 mr-1" />
                          ) : (
                    ...
                    {selectedColumns.includes("marketCap") && (
                      <TableCell className="text-gray-300">
                        {formatNumber(asset.marketCap, "currency")}
                      </TableCell>
                    )}
                    {selectedColumns.includes("volume24h") && (
                      <TableCell className="text-gray-300">
                        {formatNumber(asset.volume24h, "currency")}
                      </TableCell>
                    )}
                    {selectedColumns.includes("rank") && (
                      <TableCell className="text-gray-300">
                    ...
                    {selectedColumns.includes("macd") && (
                      <TableCell>
                        <div
                          className={`${asset.macd > 0 ? "text-green-400" : "text-red-400"}`}
                        >
